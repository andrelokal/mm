...
function initJs(){
	...
	$('.ApplyKeyTable').remove();	
	if( uri && uri.indexOf('sell.php') != -1 ){
		if( $('#datagrid').length ) ApplyKeyTable( $('#datagrid') );
	}

	/*if( pageActived.length ){

		$('#'+pageActived[2]).not('.done').load( pageActived[0], function(){
			// callback padr?
			initJs()
			if( pageActived[1] ) {
...
function ComboPessoasFisica( where, callback ){
	...
	var pessoasPF = new $PHP('fisicaController');
...
function MenuSystem(){
	var modulo = new $PHP('moduloController');
